...
The code itself will store the user creds in the a dedicated view model which also contains a login function that will be used for firebase auth.
...
The code itself uses the same viewmodel as login, which stores a potential user that will be populated by each page. This viewmodel also contains the sign up function including converting the user object to a dict to store in firebase.
